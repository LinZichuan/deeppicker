...
if len(sys.argv) < 3:
    print ("python seperate*py [path of run_*_data.star] [dir of output extracted pickle, e.g. class2d_seperated_star]")
    exit()
filename = sys.argv[1]
...
for c,stars in all_star_dict.items():
    for imagename, star_list in stars.items():
        filepath = os.path.join(basepath, imagename[:-4] + "_class%d.star"%c)
