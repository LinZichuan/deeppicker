...
import sys
if len(sys.argv) < 4:
    print ("python sca*.py [path of run*data.star] [dir of _new.star] [path of transition.py] [symbol] [classK]")
    exit()
input = sys.argv[1] # path of run_it*data.star
output = sys.argv[2] # dir of _new.star, e.g. class2dstar_recenter
trans = sys.argv[3]
symbol = sys.argv[4]
classK = int(sys.argv[5])
...
for l in fin:
    ...
    dx_tra = -dx - dx_rot
    dy_tra = -dy - dy_rot #up
    x_new = round(x + dx_tra)
    y_new = round(y + dy_tra) #down
...
for imagename, stars in image_star.items():
    ...
    with open(filepath, 'w') as fout:
        print>>fout, prefix #NOTE:print>>fout have endl, so the last word in prefix is not '\n'.
        for star in stars:
...
for imagename, stars in new_image_star.items():
    filepath = os.path.join(output, imagename[:-4] + symbol + "_recentered.star")
    with open(filepath, 'w') as fout:
        print>>fout, prefix
        for star in stars:
            if choice == 'writeclass':
                print>>fout, star[0], star[1], '-999 -999 -999', star[2]
            else:
